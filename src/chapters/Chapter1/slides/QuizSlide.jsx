...
export default function QuizSlide({ quizId, onNext, onPrev }) {
    ...
    return (
      <div className="h-full flex items-center justify-center px-4">
        <p className="text-2xl text-gray-600 text-center">
          Quiz not found.<br />
          Check quiz ID: <code className="bg-gray-100 px-2 py-1 rounded">{quizId}</code>
        </p>
      </div>
    );
  }
  ...
  // 🎉 Result Screen
  if (isComplete) {
    // Safe fallback: avoid crash if results is null/undefined
    const safeResults = results && typeof results === 'object' 
      ? results 
      : { correct: 0, total: quiz.questions.length, percentage: 0, passed: false };
    ...
    const getBadge = (percentage) => {
      ...
      return { emoji: '💪', title: 'Keep Learning!', color: 'from-gray-400 to-gray-600' };
    };

    const badge = getBadge(safeResults.percentage);
      ...
      <div className="h-full flex flex-col justify-center items-center px-4 py-6 overflow-y-auto">
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 mb-6">
          <div className="flex flex-col items-center justify-center">
            <div className="text-8xl mb-3 sm:mb-4 animate-bounce">{badge.emoji}</div>
            <div className={`bg-gradient-to-r ${badge.color} text-white p-6 sm:p-8 md:p-10 rounded-3xl shadow-2xl w-full max-w-md`}>
              <div className="text-center lg:text-left">
                ...
                <p className="text-4xl sm:text-5xl md:text-6xl font-bold mb-2">
                  {safeResults.correct}/{safeResults.total}
                </p>
                <p className="text-xl sm:text-2xl md:text-3xl font-semibold">{safeResults.percentage}%</p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
              {quiz.questions.map((question, index) => {
                ...
                return (
                  <div 
                    key={index} 
                    className="flex items-start gap-3 p-3 rounded bg-gray-50 text-sm sm:text-base"
                  >
                    {correct ? (
        ...
        <div className="flex flex-col sm:flex-row sm:justify-center gap-3 sm:gap-4 mt-4 sm:mt-6 w-full max-w-2xl">
          <button
            onClick={onPrev}
